...
//  AirGuitar
//
//  Created by Joshua Verdejo on 3/30/21.
//
...
class ViewController: UIViewController,  CBPeripheralDelegate, CBCentralManagerDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        let background = UIImage(named: "Guitar.png")!
    ...
    @IBAction func resetTouched(_ sender: Any) {
        ...
        let ds_black = [Double](repeatElement(0.0, count: tri.count*3))
        let (mesh,info) = mapSingle(ds: ds_black,pts: pts,tri: tri, w: Double(self.view.frame.width))
        mesh.tag = frameTag
        info.tag = infoTag
        mesh.center = CGPoint(x: self.view.frame.width/2, y: self.view.frame.width)
    ...
    @IBAction func strum(_ sender: Any) {
        print("strum", self.gestureLabel.text)
        playChord(chord: self.gestureLabel.text!)
    }
    ...
    private lazy var classificationRequest: VNCoreMLRequest = {
        do {
            // 2
            let model = try VNCoreMLModel(for: GestureClassifier_1().model)
            // 3
            let request = VNCoreMLRequest(model: model) { request, _ in
                if let classifications =
                    ...
                    DispatchQueue.main.async {
                        ...
                        self.confidenceLabel.text  = "[ c: " + String(round(self.confidence*10000)/100) + "% ]"
                    }
